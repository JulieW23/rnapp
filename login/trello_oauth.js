        ...
        for (i = 0; i < boards.length; i++) {
            ...
            if (lists[0]){
              ...
              .then(res => {
                ...
                for(i=0; i < lists.length; i++){
                  ...
                  [lists[i].id, lists[i].idBoard, lists[i].name, 
                  lists[i].closed, memberships1], function(err, result){ 
                    if(err){
                      // console.log("error: " + err);
                      list_nothing = 1;
                    }
                  ...
                  function(error, data, response){
                    ...
                    for(i=0; i < cards.length; i++){
                      if (cards[i].due == null){
                        ...
                        cards[i].dueComplete, cards[i].idBoard, 
                        cards[i].idList, cards[i].shortUrl, cards[i].closed, 
                        memberships1], function(err, result){  
                          if(err){
                            // console.log("error: " + err);
                            card_nothing = 1;
                          }
                        ...
                        [cards[i].id, cards[i].name, cards[i].desc, 
                        cards[i].due, cards[i].dueComplete, cards[i].idBoard, 
                        cards[i].idList, cards[i].shortUrl, cards[i].closed, 
                        memberships1], function(err, result){   
                          if(err){
                            // console.log("error: " + err);
                  ...
                  oauth.getProtectedResource("https://api.trello.com/1/lists/" 
                  + lists[i].id + "/cards?filter=closed&since=" 
                  + two_years_ago, "GET", accessToken, accessTokenSecret, 
                  function(error, data, response){
                    var cards = JSON.parse(data);
                    var card_nothing = 1;
                    for(i=0; i < cards.length; i++){
          ...
          function(error, data, response){
            ...
            async.eachSeries(actions, function(action, done){
              ...
              action.data.boardTarget.id], function(err, result){
                if(err){
                  // console.log(err);
                  action_nothing = 1;
                }
          ...
          function(error, data, response){
            ...
            async.eachSeries(actions, function(action, done){
              ...
              action.data.board.id], function(err, result){
                if(err){
                  // console.log(err);
                  action_nothing=1;
                }
